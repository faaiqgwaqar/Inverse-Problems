...
# ╔═╡ 520b12ec-5c9e-11ed-0d23-e98f55e04416
using PlutoUI, CairoMakie, CSV, DataFrames, Optim, ColorSchemes

# ╔═╡ 3cad0d98-19ee-4575-ad85-e37b61f6f315
md"# interactive parameter identification
🚀 🦫
"

# ╔═╡ 09f14837-c5cb-41d2-9c24-45ccab148c53
my_colors = ColorSchemes.Accent_3

# ╔═╡ e8499759-4b45-42b2-9e3d-e0d3d7cf325f
TableOfContents()

# ╔═╡ 8a7158a4-2992-4528-96f6-3003e9aeedb4
begin
...
# ╔═╡ cddcfb11-af1d-4065-8953-c2294813cf50
md"## read in data"

# ╔═╡ f1f8b460-d00c-42fa-ac56-c80c6e58a544
data = CSV.read("lime_temperature_time_series.csv", DataFrame)

# ╔═╡ 9ae47a66-24ba-4988-a723-3f47b31c9e59
θᵃⁱʳ = 18.56 # °C

# ╔═╡ b3b6b9bc-9a54-412c-bea0-83f3f158a942
θ₀ = data[1, "T [°C]"] # °C

# ╔═╡ df122e22-ac8e-46af-9cbd-b5e4058fba98
md"## the model"

# ╔═╡ 810d481d-c0c8-4bb0-b9de-c0227e1237a0
function θ(t::Number, params::NamedTuple)
	if t < 0.0
		return params.θ₀
	else
		return params.θ₀ + (params.θᵃⁱʳ - params.θ₀) * (1 - exp(-t / params.τ))
	end
end

# ╔═╡ 074e6f6c-79d3-47f9-a717-9bba8c93459e
md"## tuning $\tau$"

# ╔═╡ d7c1f1da-6f49-4918-a89b-33f29faa1cef
md"show residuals? $(@bind show_residuals PlutoUI.CheckBox())"

# ╔═╡ 3d88b143-7998-4013-a332-4d0e457e4bd0
md"τ slider:  $(@bind τ PlutoUI.Slider(0.0:1.0:160.0))"

# ╔═╡ de4a579e-7cc7-4d9b-8964-6455698d36a7
println("τ [min] = ", τ)

# ╔═╡ 6ae4a0d9-6d74-4507-99f4-37b3b4a30fe4
begin
	fig = Figure()
	ax  = Axis(fig[1, 1], xlabel="time, t [min]", ylabel="temperature [°C]")
	###
	#   model
	params = (θ₀=θ₀, τ=τ, θᵃⁱʳ=θᵃⁱʳ)
	ts = range(0.0, 900.0, length=200)
	lines!(ts, [θ(tᵢ, params) for tᵢ in ts], 
		color=my_colors[1], label="θ(t; τ)")
	###
	#   data
	scatter!(data[:, "t [min]"], data[:, "T [°C]"], 
		strokewidth=1, color=my_colors[2], label="{(tᵢ, θᵢ)}")
	axislegend(position=:rb)
	###
	#   residuals
	if show_residuals
		for row in eachrow(data)
			tᵢ, θᵢ = row["t [min]"], row["T [°C]"]
			θ̂ᵢ = θ(tᵢ, params)
			lines!([tᵢ, tᵢ], [θᵢ, θ̂ᵢ], linewidth=1, color=my_colors[3])
		end
	end
	fig
end

# ╔═╡ 234bc221-45c6-43b9-81c7-02f1b110c3f7
md"## the optimization routine"

# ╔═╡ 5c484813-0652-45bc-abe4-a0d800817893
function loss(τ::Float64)
	params = (θ₀=θ₀, τ=τ, θᵃⁱʳ=θᵃⁱʳ)
    ℓ = 0.0
    for row in eachrow(data)
		tᵢ, θᵢ = row["t [min]"], row["T [°C]"]
...
# ╔═╡ 1e60058a-6dac-46b9-acfb-640588cb14db
τ_opt = optimize(loss, 0.0, 200.0).minimizer
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "3289b26b79a16578a5aa0bbee2ecafb8b4ef0df1"
