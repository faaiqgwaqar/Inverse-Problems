...
using Roots, CairoMakie, ColorSchemes, Roots, Optim, Printf, PlutoUI, Random, Colors

# ╔═╡ 1d589fb8-d6c9-4461-bd1e-926c8c239327
set_theme!(
    Theme(
...
# ╔═╡ 362481f7-8ec9-423d-a190-f209a2dd7038
mutable struct Params
	t₀::Float64
	T₀::Float64
end

# ╔═╡ fa7e0a49-69e8-4f12-8f4b-a1253d56626e
begin
	Tₐ = 20.0 # °C
	τ  = 5.0 # min
end

# ╔═╡ ca9475a5-1e48-45b2-945d-426c6088a591
...
begin
	t = collect(range(-1.0, 30.0, length=150))
	push!(t, -0.001)
	push!(t, 0.001)
	sort!(t)
	...
	the_ylabel = "temperature, T(t) [°C]"
end

# ╔═╡ b5b9d1ad-92f1-43aa-8f08-49c5d09d3e71
begin
	function T(t::Number, params::Params)
		if t < params.t₀
			return params.T₀
		else
			return params.T₀ + (Tₐ - params.T₀) * (1 - exp(-(t - params.t₀) / τ)) 
		end
	end
	
	T(t::Array{Float64, 1}, params::Params) = [T(tᵢ, params) for tᵢ in t]
end
...
# ╔═╡ efa9805f-f6e6-4e38-8f11-ed979d0e8e09
md"solve forward problem $(@bind plot_trajectory PlutoUI.CheckBox(false)) show errors $(@bind viz_errors_fwd_prob PlutoUI.CheckBox(false))"

# ╔═╡ 4a8b971b-8557-4caf-9aef-0e0635169758
begin
	params = Params(0.0, 4.0)

	fig = Figure()
	ax  = Axis(fig[1, 1], 
		       xlabel=the_xlabel, 
		       ylabel=the_ylabel)
	if viz_errors_fwd_prob
		errorbars!([0.0], [params.T₀], δ, δ, whiskerwidth=10, linewidth=3)
	end
	if viz_errors_fwd_prob && plot_trajectory
		T_top = T(t, Params(params.t₀, params.T₀+δ))
		T_bot = T(t, Params(params.t₀, params.T₀-δ))
		band!(t, T_bot, T_top, color=(my_colors[1], 0.2))
		lines!(t, T_top, color=my_colors[1], linestyle=:dot)
		lines!(t, T_bot, color=my_colors[1], linestyle=:dot)
	end
	hlines!(ax, Tₐ, 
		    linestyle=:dash, label="air temperature, Tₐ", 
			linewidth=3, color=my_colors[3])
	if plot_trajectory
		lines!(t, T(t, params), color=my_colors[1])
	end
	scatter!([0.0], [params.T₀], 
		linestyle=:dash, label="initial condition, (t₀, T₀)", 
		strokewidth=2, color=my_colors[2])
	xlims!(minimum(t), maximum(t))
	axislegend(@sprintf("τ = %.1f min", τ), position=:rb)
	ylims!(the_ylims...)

	save(plot_trajectory ? "forward_problem_w_soln.pdf" : "forward_problem_setup.pdf", fig)
	fig
end
...
# ╔═╡ 45c7fa62-d4ed-4e1d-85b3-862c0c27c116
begin
	n_data = 20
	ϵ = 0.2
	Random.seed!(97330)
	t_data = range(0.0, 28.0, length=n_data) .+ randn(n_data)
	T_data = T(t_data, params) .+ ϵ * randn(n_data)
	push!(t_data, 0.0)
	push!(T_data, params.T₀)
end

# ╔═╡ 02346e98-2e26-47bb-8473-3d5c2ac652bc
md"solve inverse problem $(@bind plot_trajectory2 PlutoUI.CheckBox(false))"
...
begin
	...
	scatter!(t_data, T_data, color=my_colors[4], strokewidth=1, label="{(tᵢ, Tᵢ)}")
	xlims!(minimum(t), maximum(t))
	if plot_trajectory2
		axislegend(@sprintf("τ = %.1f min", τ), position=:rb)
	else
	...
	ylims!(the_ylims...)
	save(plot_trajectory2 ? "identify_tau.pdf" : "identify_tau_setup.pdf", fig2)
	fig2
end
...
begin
	...
	Colorbar(fig3[1,2], limits=(0, 20), colormap=my_colormap, label="initial temperature T₀ [°C]")
	text!(@sprintf("τ = %.1f min", τ), position=(15, 3))
	ylims!(the_ylims...)
	xlims!(minimum(t), maximum(t))
	# axislegend(position=:rc)
	save("range_of_initial_conditions.pdf", fig3)
	fig3
end

# ╔═╡ 82b478ac-6156-48ca-8f59-206388e255a7
function find_T₀(t′, T′, params::Params)
	these_params = deepcopy(params)
	function g(T₀)
		these_params.T₀ = T₀
		return T′ - T(t′, these_params)
	end
...
begin
	fig5 = Figure(resolution = (500, 400))
	ax5  = Axis(fig5[1, 1], 
			   xlabel=the_xlabel, 
			   ylabel=the_ylabel,
	...
	T₀_ci  = [find_T₀(t′, T′ - δ, _params), find_T₀(t′, T′ + δ, _params)]
	T_bounds = []
	for T₀ in T₀_ci
		__params = deepcopy(_params)
		__params.T₀ = T₀
		push!(T_bounds, T(t_filtered, __params))
		if viz_soln & viz_errors
			lines!(t_filtered, T(t_filtered, __params), 
				   color=my_colors[1],
				   linestyle=:dot
			)
		end
	...
	if viz_soln & viz_errors
		band!(t_filtered, T_bounds[1], T_bounds[2], color=(my_colors[1], 0.2))
	end
	hlines!(ax5, Tₐ, 
		linestyle=:dash, label="air temperature, Tₐ", 
		linewidth=3, color=my_colors[3])
	...
	scatter!([t′], [T′], 
		color=my_colors[1], strokewidth=2,
		label="(t′, T(t′))"
	)

	if viz_soln
		lines!(t_filtered, T(t_filtered, _params), 
			   color=my_colors[1]
		)
		if viz_errors
			errorbars!(zeros(2), T₀, T₀ - T₀_ci[1], T₀_ci[2]-T₀, whiskerwidth=10, linewidth=3)
		end
		scatter!([0.0], [T₀], 
			linestyle=:dash, label="initial condition, (t₀, T₀)", 
			strokewidth=2, color=my_colors[2])
	...
	xlims!(minimum(t), maximum(t))
	axislegend(@sprintf("τ = %.1f min", τ), position=:rb)
	pdf_savename = "inverse_ic_"
	if t′ < 7
		pdf_savename *= "_t_prime_small"
	else
		pdf_savename *= "_t_prime_large"
	end
	if viz_soln
		pdf_savename *= "_w_soln"
	end
	if viz_errors
		pdf_savename *= "_w_errors"
	end
	if save_the_fig
		save(pdf_savename * ".pdf", fig5)
	end
	fig5
end
